...
<div class="flex min-h-screen flex-col md:flex-row">
	...
	<div class="flex min-h-screen w-full items-center justify-center bg-background p-4 md:w-1/2">
		<div class="w-full max-w-md space-y-8 rounded-xl border border-border/50 bg-card p-8 shadow-lg">
			...
			<div class="space-y-3">
				<form action="?/loginWithFacebook" method="post" use:enhance>
					<Button 
						variant="outline" 
				...
				<form action="?/loginWithGoogle" method="POST" use:enhance>
					<Button
						variant="outline"
						class="w-full cursor-pointer border-border hover:bg-accent hover:text-accent-foreground transition-all duration-200"
						type="submit"
					>
						<svg class="mr-2 h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
							<path
